...
class VAE(nn.Module):
    ...
    def reparameterize(self, mu, logvar,epsil,no_sample=False):
        
        std = logvar.mul(0.5).exp()
        if not no_sample:
            #Variable(std.data.new(std.size()).normal())
    ...
    def forward(self, x,no_sample = False,epso=1):    
        mu, logvar = self.encode(x)
        z,eps = self.reparameterize(mu, logvar,epso,no_sample)
        return self.classifier_encoder(z),mu,logvar,eps
